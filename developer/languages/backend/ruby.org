...
| Books                            | Chapter                                                   | late                                    |
|----------------------------------+-----------------------------------------------------------+-----------------------------------------|
| Programming Ruby                 | 5 Sharing Functionality: Inheritance, Modules, and Mixins | Instance Variables in Mixins            |
...
** Container
#+begin_src shell
docker run --rm -v $(pwd):/app/ -w /app ruby bundle install && mkdir -pv ~/.config/pak && cp docs/config_examples/*.yaml ~/.config/pak/ && rake
#+end_src
